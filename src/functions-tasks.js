...
function getPolynom(...args) {
  ...
  if (args.length === 2) {
    buffer = (param) => args[0] * param + args[1];
  }
  if (args.length === 3) {
    buffer = (param) => args[0] * param ** 2 + args[1] * param + args[2];
  }
  return buffer;
}
...
function logger(func, logFunc) {
  return (...args) => {
    const argsToStr = JSON.stringify(args).slice(1, -1);
    logFunc(`${func.name}(${argsToStr}) starts`);
    const resLog = func(...args);
    logFunc(`${func.name}(${argsToStr}) ends`);
    return resLog;
  };
}
